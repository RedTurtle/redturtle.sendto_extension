...
if restrict != "users":
    try:
        adv_search_view = context.restrictedTraverse('adv_search_groups')
        groups = adv_search_view.searchGroups(searchstring)
        retlist = retlist + [groups_tool.getGroupById(group['id']) for group in groups]
